...
class SoundChooserAdapter(
    ...
    private fun checkAction(item: Sound, vh: ItemVH) {
        ...
        vh.action.setImageResource(if (dled) R.drawable.ic_times_play else R.drawable.ic_times_download)
    }
    ...
    private fun playPause(vh: ItemVH, item: Sound) {
        if (vh.player == null) {
            resetAudios()
            vh.action.setImageResource(R.drawable.ic_times_pause)
    ...
    class ItemVH internal constructor(parent: ViewGroup) : RecyclerView.ViewHolder(
        LayoutInflater.from(parent.context).inflate(R.layout.times_sound_chooser_item, parent, false)
    ) {
        ...
        fun resetAudio() {
            if (seekbar.visibility == View.VISIBLE) {
                action.setImageResource(R.drawable.ic_times_play)
            }
