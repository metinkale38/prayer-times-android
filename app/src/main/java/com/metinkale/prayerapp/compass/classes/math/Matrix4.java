...
public class Matrix4 implements Serializable {
    ...
    private static Vector3 l_vez = new Vector3();
    private static Vector3 l_vex = new Vector3();
    private static Vector3 l_vey = new Vector3();
    private static Vector3 right = new Vector3();
    private static Vector3 tmpForward = new Vector3();
    private static Vector3 tmpUp = new Vector3();
     ...
     * from {@link Matrix4#val}. This is the same as
     ...
     * to hold a 4x4 column major matrix as you can get from {@link Matrix4#val}
     * . The vector array is assumed to hold a 3-component vector, with x being
     * the first element, y being the second and z being the last component. The
     * result is stored in the vector array. This is the same as
    ...
    /**
     * Multiplies the vectors with the given matrix. The matrix array is assumed
     * to hold a 4x4 column major matrix as you can get from {@link Matrix4#val}
     * . The vectors array is assumed to hold 3-component vectors. Offset
     * specifies the offset into the array where the x-component of the first
     * vector is located. The numVecs parameter specifies the number of vectors
     * stored in the vectors array. The stride parameter specifies the number of
     * floats between subsequent vectors and must be >= 3. This is the same as
    ...
    public static native void mulVec(float[] mat, float[] vecs, int offset, int numVecs, int stride); /*
             * float* vecPtr = vecs + offset; for(int i
			 * = 0; i < numVecs; i++) {
			 * matrix4_mulVec(mat, vecPtr); vecPtr +=
			 * stride; }
			 */
     ...
     * get from {@link Matrix4#val}. The vector array is assumed to hold a
     ...
     * This is the same as {@link Vector3#prj(Matrix4)}.
     *
     * @param mat the matrix
     * @param vec the vector.
     */
    ...
    public static native void prj(float[] mat, float[] vecs, int offset, int numVecs, int stride); /*
             * float* vecPtr = vecs + offset; for(int i
			 * = 0; i < numVecs; i++) {
			 * matrix4_proj(mat, vecPtr); vecPtr +=
			 * stride; }
			 */

    /**
     * Multiplies the vector with the top most 3x3 sub-matrix of the given
     * matrix. The matrix array is assumed to hold a 4x4 column major matrix as
     * you can get from {@link Matrix4#val}. The vector array is assumed to hold
     * a 3-component vector, with x being the first element, y being the second
     * and z being the last component. The result is stored in the vector array.
     * This is the same as {@link Vector3#rot(Matrix4)}.
     *
     * @param mat the matrix
     * @param vec the vector.
     */
    ...
    /**
     * Multiplies the vectors with the top most 3x3 sub-matrix of the given
     * matrix. The matrix array is assumed to hold a 4x4 column major matrix as
     * you can get from {@link Matrix4#val}. The vectors array is assumed to
     * hold 3-component vectors. Offset specifies the offset into the array
     * where the x-component of the first vector is located. The numVecs
     * parameter specifies the number of vectors stored in the vectors array.
     * The stride parameter specifies the number of floats between subsequent
    ...
    public static native void rot(float[] mat, float[] vecs, int offset, int numVecs, int stride); /*
             * float* vecPtr = vecs + offset; for(int i
			 * = 0; i < numVecs; i++) { matrix4_rot(mat,
			 * vecPtr); vecPtr += stride; }
			 */
    ...
    /**
     * Computes the determinante of the given matrix. The matrix array is
     * assumed to hold a 4x4 column major matrix as you can get from
     * {@link Matrix4#val}.
     *
     * @param values the matrix values.
    ...
    /**
     * Inverts the matrix. Throws a {@link RuntimeException} in case the matrix
     * is not invertible. Stores the result in this matrix.
     *
     * @return This matrix for the purpose of chaining methods together.
     */
    Matrix4 inv() {
     ...
     * overwritting it with an identity and then setting the translation vector
     * in the 4th column and the scaling vector in the diagonal.
     *
    ...
    public Matrix4 setToLookAt(Vector3 position, Vector3 target, Vector3 up) {
        ...
        mul(tmpMat.setToTranslation(position.tmp().mul(-1)));
	 ...
	 * * val[M12] * val[M03] - val[M20] * val[M31] * val[M12] * val[M03] -
	 * val[M30] * val[M11] val[M22] * val[M03] + val[M10] * val[M31] * val[M22]
	 * * val[M03] + val[M20] * val[M11] * val[M32] * val[M03] - val[M10]
	 * val[M21] * val[M32] * val[M03] - val[M30] * val[M21] * val[M02] *
	 * val[M13] + val[M20] * val[M31] * val[M02] * val[M13] + val[M30] *
	 * val[M01] * val[M22] * val[M13] - val[M00] * val[M31] * val[M22] *
	 * val[M13] - val[M20] * val[M01] * val[M32] val[M13] + val[M00] * val[M21]
	 * * val[M32] * val[M13] + val[M30] * val[M11] * val[M02] * val[M23] -
	 * val[M10] * val[M31] val[M02] * val[M23] - val[M30] * val[M01] * val[M12]
	 * * val[M23] + val[M00] * val[M31] * val[M12] * val[M23] + val[M10]
	 * val[M01] * val[M32] * val[M23] - val[M00] * val[M11] * val[M32] *
	 * val[M23] - val[M20] * val[M11] * val[M02] * val[M33] + val[M10] *
	 * val[M21] * val[M02] * val[M33] + val[M20] * val[M01] * val[M12] *
	 * val[M33] - val[M00] * val[M21] * val[M12] val[M33] - val[M10] * val[M01]
	 ...
	 * val[M23] * val[M31] - val[M13] * val[M22] * val[M31] + val[M13] *
	 * val[M21] * val[M32] - val[M11] val[M23] * val[M32] - val[M12] * val[M21]
	 * * val[M33] + val[M11] * val[M22] * val[M33]; tmp[M01] = val[M03] *
	 * val[M22] * val[M31] - val[M02] * val[M23] * val[M31] - val[M03] *
	 * val[M21] * val[M32] + val[M01] val[M23] * val[M32] + val[M02] * val[M21]
	 * * val[M33] - val[M01] * val[M22] * val[M33]; tmp[M02] = val[M02] *
	 * val[M13] * val[M31] - val[M03] * val[M12] * val[M31] + val[M03] *
	 * val[M11] * val[M32] - val[M01] val[M13] * val[M32] - val[M02] * val[M11]
	 * * val[M33] + val[M01] * val[M12] * val[M33]; tmp[M03] = val[M03] *
	 * val[M12] * val[M21] - val[M02] * val[M13] * val[M21] - val[M03] *
	 * val[M11] * val[M22] + val[M01] val[M13] * val[M22] + val[M02] * val[M11]
	 * * val[M23] - val[M01] * val[M12] * val[M23]; tmp[M10] = val[M13] *
	 * val[M22] * val[M30] - val[M12] * val[M23] * val[M30] - val[M13] *
	 * val[M20] * val[M32] + val[M10] val[M23] * val[M32] + val[M12] * val[M20]
	 * * val[M33] - val[M10] * val[M22] * val[M33]; tmp[M11] = val[M02] *
	 * val[M23] * val[M30] - val[M03] * val[M22] * val[M30] + val[M03] *
	 * val[M20] * val[M32] - val[M00] val[M23] * val[M32] - val[M02] * val[M20]
	 * * val[M33] + val[M00] * val[M22] * val[M33]; tmp[M12] = val[M03] *
	 * val[M12] * val[M30] - val[M02] * val[M13] * val[M30] - val[M03] *
	 * val[M10] * val[M32] + val[M00] val[M13] * val[M32] + val[M02] * val[M10]
	 * * val[M33] - val[M00] * val[M12] * val[M33]; tmp[M13] = val[M02] *
	 * val[M13] * val[M20] - val[M03] * val[M12] * val[M20] + val[M03] *
	 * val[M10] * val[M22] - val[M00] val[M13] * val[M22] - val[M02] * val[M10]
	 * * val[M23] + val[M00] * val[M12] * val[M23]; tmp[M20] = val[M11] *
	 * val[M23] * val[M30] - val[M13] * val[M21] * val[M30] + val[M13] *
	 * val[M20] * val[M31] - val[M10] val[M23] * val[M31] - val[M11] * val[M20]
	 * * val[M33] + val[M10] * val[M21] * val[M33]; tmp[M21] = val[M03] *
	 * val[M21] * val[M30] - val[M01] * val[M23] * val[M30] - val[M03] *
	 * val[M20] * val[M31] + val[M00] val[M23] * val[M31] + val[M01] * val[M20]
	 * * val[M33] - val[M00] * val[M21] * val[M33]; tmp[M22] = val[M01] *
	 * val[M13] * val[M30] - val[M03] * val[M11] * val[M30] + val[M03] *
	 * val[M10] * val[M31] - val[M00] val[M13] * val[M31] - val[M01] * val[M10]
	 * * val[M33] + val[M00] * val[M11] * val[M33]; tmp[M23] = val[M03] *
	 * val[M11] * val[M20] - val[M01] * val[M13] * val[M20] - val[M03] *
	 * val[M10] * val[M21] + val[M00] val[M13] * val[M21] + val[M01] * val[M10]
	 * * val[M23] - val[M00] * val[M11] * val[M23]; tmp[M30] = val[M12] *
	 * val[M21] * val[M30] - val[M11] * val[M22] * val[M30] - val[M12] *
	 * val[M20] * val[M31] + val[M10] val[M22] * val[M31] + val[M11] * val[M20]
	 * * val[M32] - val[M10] * val[M21] * val[M32]; tmp[M31] = val[M01] *
	 * val[M22] * val[M30] - val[M02] * val[M21] * val[M30] + val[M02] *
	 * val[M20] * val[M31] - val[M00] val[M22] * val[M31] - val[M01] * val[M20]
	 * * val[M32] + val[M00] * val[M21] * val[M32]; tmp[M32] = val[M02] *
	 * val[M11] * val[M30] - val[M01] * val[M12] * val[M30] - val[M02] *
	 * val[M10] * val[M31] + val[M00] val[M12] * val[M31] + val[M01] * val[M10]
	 * * val[M32] - val[M00] * val[M11] * val[M32]; tmp[M33] = val[M01] *
	 * val[M12] * val[M20] - val[M02] * val[M11] * val[M20] + val[M02] *
	 * val[M10] * val[M21] - val[M00] val[M12] * val[M21] - val[M01] * val[M10]
	 * * val[M22] + val[M00] * val[M11] * val[M22];
	 *
	 ...
	 *
	 * static inline void matrix4_mulVec(float* mat, float* vec) { float x =
	 * vec[0] * mat[M00] + vec[1] * mat[M01] + vec[2] * mat[M02] + mat[M03];
	 * float y = vec[0] * mat[M10] + vec[1] * mat[M11] + vec[2] * mat[M12] +
	 * mat[M13]; float z = vec[0] * mat[M20] + vec[1] * mat[M21] + vec[2] *
	 * mat[M22] + mat[M23]; vec[0] = x; vec[1] = y; vec[2] = z; }
	 *
	 * static inline void matrix4_proj(float* mat, float* vec) { float inv_w =
	 * 1.0f / (vec[0] * mat[M30] + vec[1] * mat[M31] + vec[2] * mat[M32] +
	 * mat[M33]); float x = (vec[0] * mat[M00] + vec[1] * mat[M01] + vec[2] *
	 * mat[M02] + mat[M03]) * inv_w; float y = (vec[0] * mat[M10] + vec[1] *
	 * mat[M11] + vec[2] * mat[M12] + mat[M13]) * inv_w; float z = (vec[0] *
	 * mat[M20] + vec[1] * mat[M21] + vec[2] * mat[M22] + mat[M23]) * inv_w;
	 * vec[0] = x; vec[1] = y; vec[2] = z; }
	 *
	 * static inline void matrix4_rot(float* mat, float* vec) { float x = vec[0]
	 * * mat[M00] + vec[1] * mat[M01] + vec[2] * mat[M02]; float y = vec[0] *
	 * mat[M10] + vec[1] * mat[M11] + vec[2] * mat[M12]; float z = vec[0] *
	 * mat[M20] + vec[1] * mat[M21] + vec[2] * mat[M22]; vec[0] = x; vec[1] = y;
	 * vec[2] = z; }
	 */
