...
public class Sounds {
    ...
    public static class Sound implements Serializable {
        ...
        public File getFile() {
            return new File(url.replace(App.API_URL+"/", App.getContext().getExternalFilesDir(null).getAbsolutePath()));
        }
        ...
        public boolean needsRedownload(List<Times.Alarm> alarms) {
            if (getFile().exists()) {
                ...
                if (md5 == null) {
                    try {
                        String file = this.url.substring(this.url.indexOf("sounds/") + 7);
                        URL url = new URL("http://namazvakti.tk/sounds/md5.php?file=" + file);
